              ...
              <div className="space-y-5">
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {["八字命理","紫微斗數","塔羅占卜","生命靈數","西洋占星"].map((tag) => (
                    <span key={tag} className="text-xs font-sans px-3 py-1 rounded-full"
                      style={{ background: "rgba(184,144,42,0.10)", color: "#8A6A1A", border: "1px solid rgba(184,144,42,0.25)" }}>
                      {tag}
                    </span>
                  ))}
                </div>
          ...
          <div className="max-w-5xl mx-auto px-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                ...
                <div key={s.label} className="text-center space-y-2 p-6 rounded-2xl"
                  style={{ background: "#fff", border: "1px solid rgba(184,144,42,0.12)", boxShadow: "0 2px 12px rgba(26,45,69,0.04)" }}>
                  <p className="font-serif font-bold leading-none" style={{ fontSize: "2.4rem", color: "#B8902A" }}>
                    {s.num}<span className="text-xl">{s.unit}</span>
                  </p>
                  <p className="font-sans text-sm" style={{ color: "#5A6E82" }}>{s.label}</p>
                </div>
              ))}
            </div>
            ...
            {/* Milestones */}
            <div className="mt-14">
              <h3 className="font-serif text-xl font-semibold text-center mb-10" style={{ color: "#1A2D45" }}>專業歷程</h3>
              <div className="relative">
                <div className="absolute left-1/2 top-0 bottom-0 w-px -translate-x-1/2 hidden md:block"
                  style={{ background: "linear-gradient(to bottom, transparent, #B8902A 15%, #B8902A 85%, transparent)" }} />
                  ...
                  {[
                    ...
                    <div key={m.year} className={`flex items-start gap-6 md:gap-0 ${i % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`}>
                      <div className={`md:w-[calc(50%-2rem)] space-y-1 ${i % 2 === 0 ? "md:text-right md:pr-8" : "md:text-left md:pl-8"}`}>
                        <p className="font-sans text-xs font-semibold tracking-widest" style={{ color: "#B8902A" }}>{m.year}</p>
                        <p className="font-serif text-base font-semibold" style={{ color: "#1A2D45" }}>{m.title}</p>
                        <p className="font-sans text-sm leading-relaxed" style={{ color: "#5A6E82" }}>{m.desc}</p>
                      </div>
                      <div className="relative flex-shrink-0 z-10 md:mx-auto">
                        <div className="w-4 h-4 rounded-full border-2 mt-1"
                          style={{ background: "#B8902A", borderColor: "#F5F1EA" }} />
                      </div>
